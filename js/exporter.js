...
(function (App) {
  ...
  function cube() {
    ...
    deps.ensurePrep3d(function (prep) {
      ...
      var lines = [
        "quantum-field-molecules: " + (deps.getPreset() ? deps.getPreset().formula + " " + deps.getPreset().name.en : "custom molecule"),
      ...
      for (var x = 0; x < N; x++) {
        for (var y = 0; y < N; y++) {
          ...
          for (var z = 0; z < N; z++) {
            ...
            if (row.length === 6 || z === N - 1) { out.push(row.join("  ")); row = []; }
          }
        }
      }
      download(new Blob([lines.join("\n") + "\n" + out.join("\n") + "\n"], { type: "text/plain" }),
        baseName() + "-" + modeTag(mode) + ".cube");
    });
  }
  ...
  function json() {
    ...
    var payload = {
      ...
      molecule: preset ? preset.formula + " " + preset.name.en : "custom",

...
(function (App) {
  ...
  d.helpSections = [
    ...
    {
      id: "finder", title: "Property finder (inverse design, experimental)",
      body: "Most of the application goes molecule \u2192 properties. This panel goes the other way: you name the properties you want and it proposes the closest molecule it can reach. It is a bounded, honest heuristic, not a generative design engine.\n\nStarting from the current molecule (the seed), it substitutes one atom at a time - heavy atoms over B/C/N/O/F, hydrogens swapped with fluorine - relaxes each candidate's geometry, runs Hartree-Fock (STO-3G) and ranks the survivors by how close they land to your targets. Tick any combination of dipole moment, HOMO-LUMO gap, Koopmans ionization potential and shell (closed/open); the lower the score, the closer to the targets. An odd electron count is handled automatically as a UHF doublet, so substitutions that change the parity still work.\n\nCandidates are HF/STO-3G guesses from a tiny search space - good for building structure-property intuition, not validated designs. Press \u201cload\u201d on a result to drop it into the editor, then \u201coptimize geometry\u201d for a real minimum and a full calculation in your chosen basis."
    },
    {
  ...
  d.glossary = [
    ...
    { id: "inverse-design", term: "Inverse design (property finder)",
      body: "The reverse of the usual workflow: instead of computing properties of a given molecule, you specify target properties and search for a structure that matches. Here it is a bounded heuristic - single-atom substitutions of the current molecule, each relaxed and ranked by Hartree-Fock observables (dipole, HOMO-LUMO gap, ionization potential, shell). It suggests candidates to build structure-property intuition; it is not a validated generative-design method." },
